...
extern MY_HIGHP_OR_MEDIUMP vec2 prophecy;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float timer = prophecy.y;

    // Image depths = prophecy.z;

    float depths_xcoord = mod((card_pixel_coords.x/card_size.y) - (timer/20.0), 1);
    float depths_ycoord = mod((card_pixel_coords.y/card_size.y) - (timer/25.0), 1);

    vec4 depths_tex = Texel(mask_texture, vec2(depths_xcoord, depths_ycoord));
    vec4 depths_col = depths_tex.rgba;


    float depths2_xcoord = mod((card_pixel_coords.x/card_size.y) + (timer/30.0), 1);
    float depths2_ycoord = mod((card_pixel_coords.y/card_size.y) + (timer/35.0), 1);

    vec4 depths2_tex = Texel(mask_texture, vec2(depths2_xcoord, depths2_ycoord));
    vec4 depths2_col = depths2_tex.rgba;

    vec4 depths_total_col = (depths_col * 2/3) + (depths2_col * 1/3);
    ...
    float brightness = (0.2126*pixelColor.r) + (0.7152*pixelColor.g) + (0.0722*pixelColor.b);

    pixelColor.r = depths_total_col.r * pixelColor.r * 2;
    pixelColor.g = depths_total_col.g * pixelColor.g * 2;
    pixelColor.b = depths_total_col.b * pixelColor.b * 2;
